...
char *empty_string(void)
{
  ...
  if (emptyString == NULL) {
    perror("empty_string() unimplemented.\n");
    exit(EXIT_FAILURE);
  }
...
char *clone(const char *str)
{
  ...
  char *cloneString = calloc(size + 1, sizeof(char));

  if (cloneString == NULL) {
    perror("clone() unimplemented.\n");
    exit(EXIT_FAILURE);
  }


  return strcpy(cloneString, str);
}
...
char *push_string(char *current, const char *append)
{
  ...
  size_t finalLen = currentLen + appendLen + 1;

  current = realloc(current, finalLen);


  if (current == NULL) {
    perror("push_string() unimplemented.\n");
    exit(EXIT_FAILURE);
  }

  strncat(current, append, appendLen);
